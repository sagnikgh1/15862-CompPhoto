...
def parse_args():
    ...
    parser.add_argument('--ambient_path', default='../data/museum/museum_ambient.png', help='Path to ambient image')

    parser.add_argument('--flash_path', default='../data/museum/museum_flash.png', help='Path to flash image')
...
if __name__=="__main__":
    ...
    # Differentiate and reintegrate image
    I_star = []
    ...
    for i in range(3): # Loop over color channels
        div_im = laplacian(im_ambient[:,:,i])
        I_init = np.zeros_like(div_im)
        I_star.append(poissonSolver(div_im, I_init, B, im_ambient[:,:,i] * (1 - B), 1e-3, 1000))
    I_star = np.stack(I_star, axis=-1)

    plt.imsave("../data/reintegrated.jpg", (I_star*255).astype(np.uint8))
    ...
    plt.imsave("../data/fused.jpg", (phi_star*255).astype(np.uint8))
